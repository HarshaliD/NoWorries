...
def update_state(state: EmotionalState, user_message: str):
    ...
    # No hyperarousal keywords in this message — step down if we were in it.
    # This prevents the engine from staying stuck in hyperarousal forever.
    if state.reaction_type == "hyperarousal":
        state.reaction_type = state.previous_reaction
        state.intensity = 1 if state.previous_reaction else 0
        state.previous_reaction = None
